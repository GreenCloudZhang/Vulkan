...
public:
	...
	{		
		...
		if (vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute)
		{
			...
			bufferMemoryBarriers[0] = vks::initializers::bufferMemoryBarrier();
			bufferMemoryBarriers[0].buffer = model.vertices.buffer;
			bufferMemoryBarriers[1] = vks::initializers::bufferMemoryBarrier();
			bufferMemoryBarriers[1].buffer = model.indices.buffer;
			bufferMemoryBarriers[0].srcAccessMask = VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
			bufferMemoryBarriers[0].dstAccessMask = 0;
			bufferMemoryBarriers[0].srcQueueFamilyIndex = vulkanDevice->queueFamilyIndices.graphics;
			bufferMemoryBarriers[0].dstQueueFamilyIndex = vulkanDevice->queueFamilyIndices.compute;
			bufferMemoryBarriers[0].size = VK_WHOLE_SIZE;
			bufferMemoryBarriers[1].srcAccessMask = VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
			bufferMemoryBarriers[1].dstAccessMask = 0;
			bufferMemoryBarriers[1].srcQueueFamilyIndex = vulkanDevice->queueFamilyIndices.graphics;
			bufferMemoryBarriers[1].dstQueueFamilyIndex = vulkanDevice->queueFamilyIndices.compute;
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			if (vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute)
			{
				...
				bufferMemoryBarriers[0].srcAccessMask = 0;
				bufferMemoryBarriers[0].dstAccessMask = VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
				bufferMemoryBarriers[0].srcQueueFamilyIndex = vulkanDevice->queueFamilyIndices.compute;
				bufferMemoryBarriers[0].dstQueueFamilyIndex = vulkanDevice->queueFamilyIndices.graphics;
				bufferMemoryBarriers[0].size = VK_WHOLE_SIZE;

				bufferMemoryBarriers[1].srcAccessMask = 0;
				bufferMemoryBarriers[1].dstAccessMask = VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
				bufferMemoryBarriers[1].srcQueueFamilyIndex = vulkanDevice->queueFamilyIndices.compute;
				bufferMemoryBarriers[1].dstQueueFamilyIndex = vulkanDevice->queueFamilyIndices.graphics;
				bufferMemoryBarriers[1].size = VK_WHOLE_SIZE;
				vkCmdPipelineBarrier(
					drawCmdBuffers[i],
	...
	void buildComputeCommandBuffer()
	{
		...
		if (vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute)
		{
			...
			bufferMemoryBarriers[0].srcAccessMask = 0;
			bufferMemoryBarriers[0].dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
			bufferMemoryBarriers[0].srcQueueFamilyIndex = vulkanDevice->queueFamilyIndices.graphics;
			bufferMemoryBarriers[0].dstQueueFamilyIndex = vulkanDevice->queueFamilyIndices.compute;
			bufferMemoryBarriers[0].size = VK_WHOLE_SIZE;
	...
	void prepareGraphics()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "gi/texture.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "gi/texture.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		VkDescriptorBufferInfo verticesDescriptor{};
		VkDescriptorBufferInfo indicesDescriptor{};
		verticesDescriptor.offset = 0;
		verticesDescriptor.range = VK_WHOLE_SIZE;
		verticesDescriptor.buffer = model.vertices.buffer;

		indicesDescriptor.offset = 0;
		indicesDescriptor.range = VK_WHOLE_SIZE;
		indicesDescriptor.buffer = model.indices.buffer;
		...
		std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2, &verticesDescriptor),
			vks::initializers::writeDescriptorSet(compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 3, &indicesDescriptor),
		};
		...
		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "gi/uvlmshow.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
	...
	{
		compute.uniformData.vertexNum = model.vertices.count;
		compute.uniformData.primitivesNum = model.indices.count / 3;
	...
	void loadAssetAndBuildLightmapUV() 
	{
		vkglTF::memoryPropertyFlags = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
		//vkglTF::isCalcLMUV = true;//BUILD LIGHTMAP UV
		const uint32_t glTFLoadingFlags = vkglTF::FileLoadingFlags::PreTransformVertices | vkglTF::FileLoadingFlags::PreMultiplyVertexColors | vkglTF::FileLoadingFlags::FlipY;
		model.loadFromFile(getAssetPath() + "models/deer2.gltf", vulkanDevice, queue, glTFLoadingFlags);
	}

	void prepare()
	{
		VulkanExampleBase::prepare();
		loadAssetAndBuildLightmapUV();
		prepareStorageImage();
